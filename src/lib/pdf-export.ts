/* Wattouna Edge PDF Blueprint exporter — TypeScript source of truth.
   Compiled via:
     npx esbuild src/lib/pdf-export.ts --format=esm --outfile=public/vendor/pdf-export.js
   Zero dependencies, 100% offline: hand-rolled PDF 1.4 writer (Helvetica
   Latin text + vector rules + embedded JPEG snapshot). Arabic is kept OUT of
   the PDF body on purpose (correct shaping needs font engines); the blueprint
   uses EN/FR technical labels, matching international schematic practice. */

export interface BlueprintInput {
  title: string;
  purpose: string;
  svgSnapshot: string; // standalone <svg …> markup (xmlns required)
...
function strBytes(s: string): Uint8Array { return enc.encode(s); }
...
/** Parse JPEG SOF marker → {w, h}. Throws on invalid data. */
export function jpegSize(data: Uint8Array): { w: number; h: number } {
  if (data[0] !== 0xff || data[1] !== 0xd8) throw new Error('not-jpeg');
  let i = 2;
...
function svgToJpegBytes(svg: string, w: number, h: number): Promise<Uint8Array> {
...
function latin(s: string): string {
  // Helvetica WinAnsi: strip non-latin (Arabic kept out by design)
  return s.replace(/[^\x20-\x7e\u00a0-\u00ff€]/g, '').slice(0, 160);
}

interface Page { ops: string[]; y: number }
const PW = 595, PH = 842, M = 48; // A4 portrait points

export async function generateBlueprint(input: BlueprintInput): Promise<Blob> {
  const pages: Page[] = [{ ops: [], y: PH - M }];
  const cur = () => pages[pages.length - 1];
  const need = (h: number) => {
    if (cur().y - h < M) pages.push({ ops: [], y: PH - M });
  };
  const text = (t: string, size: number, r = 0.9, g = 0.93, b = 0.97, font = 'Helvetica-Bold') => {
    need(size + 6);
    const p = cur();
    p.ops.push(`BT /${font} ${size} Tf ${r.toFixed(2)} ${g.toFixed(2)} ${b.toFixed(2)} rg ${M} ${p.y.toFixed(1)} Td (${pdfEsc(latin(t))}) Tj ET`);
    p.y -= size + 6;
  };
  const rule = (color: [number, number, number] = [0.96, 0.62, 0.04]) => {
    const p = cur();
    p.ops.push(`${color.join(' ')} RG 1.2 w ${M} ${p.y.toFixed(1)} m ${PW - M} ${p.y.toFixed(1)} l S`);
    p.y -= 10;
  };
  const gap = (h: number) => { cur().y -= h; };
  ...
  // ---- assemble PDF ----
  const objs: Array<{ num: number; body: Uint8Array }> = [];
  let n = 1;
  const imgId = n++, contentIds: number[] = [];
  const pageIds: number[] = [];
  pages.forEach(() => { contentIds.push(n++); pageIds.push(n++); });
  ...
  // fix font references used above (Helvetica→F1, Helvetica-Bold→F2)
