...
function createDirector(W = 960, H = 540) {
  ...
  function frame(nodes, wires, caption) {
    ...
    nodes.forEach((n, i) => {
      ...
      g.lineWidth = 3;
      g.beginPath();
      g.roundRect ? g.roundRect(x, y, 180, 90) : g.rect(x, y, 180, 90);
      g.fill();
      g.stroke();
      g.fillStyle = "#e6edf7";
    ...
    async start(opts = {}) {
      ...
      if (opts.mic) {
        try {
          ...
          opts.onLog?.("\u{1F399}\uFE0F mic track muxed");
        } catch {
          opts.onLog?.("\u26A0\uFE0F mic denied \u2014 video only");
        }
      }
      const mime = ["video/webm;codecs=vp9,opus", "video/webm;codecs=vp8,opus", "video/webm"].find((m) => window.MediaRecorder?.isTypeSupported?.(m)) || "";
      chunks = [];
      rec = new MediaRecorder(stream, mime ? { mimeType: mime, videoBitsPerSecond: 4e6 } : void 0);
      rec.ondataavailable = (e) => {
        if (e.data && e.data.size) chunks.push(e.data);
      };
      rec.start(500);
      recording = true;
      opts.onLog?.("\u{1F534} recording" + (mime ? ` (${mime.split(";")[0]})` : ""));
    },
    async stop() {
...
async function runChoreography(o) {
  const spoken = [];
